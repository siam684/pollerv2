...
if(isset($_POST['functionName']))
{
	...
	if($_POST['functionName']=='db_addColumn')
	{
		...
		$r->setNewLogFileLocation($filename);
		$cols = $_POST['columns'];
		echo $r->addColumn($tableName, $cols);
		$r->closeIt();
	}
	
	if($_POST['functionName']=='db_clearRecords')
	{
		$r = new DbRequests($_SERVER['SCRIPT_FILENAME']);
		$tableName = $_POST['tableName'];
		echo $r->clearRecords($tableName);
		$r->closeIt();
	}
...
class DbRequests
{
	...
	function setNewLogFileLocation($folderName)
	{
		global $logger;
		$logger = fopen($folderName."/master_log_file.txt","a");
		fclose($log);
	}
	
	function clearRecords($tableName)
	{
		global $conn,$globalPageName;        
        		
		$sql = "DELETE FROM ".$tableName." WHERE loginpw is null";
		//echo $sql."<br>";
